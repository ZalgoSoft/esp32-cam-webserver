...
extern void print_client_ip(httpd_req_t *req);
...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  Serial.println( "capture_handler CPU ");
  //ESP_LOGI(TAG, xPortGetCoreID());
...
static esp_err_t dump_handler(httpd_req_t *req) {
  ...
  char * d = dumpOut;
  // Header
  d += sprintf(d, "<!DOCTYPE html><html lang=en><head><meta charset=\"utf-8\">\n");
  ...
  d += sprintf(d, "PTZ:  X: %i, Y: %i, step degree value: %i<br>\n", ptz_x, ptz_y , Servo_Step);
  d += sprintf(d, "PTZ:  Pin control for Servo1: %i, pin control for Servo2: %i<br>\n", Servo1Pin, Servo2Pin);
  if (psramFound()) {
