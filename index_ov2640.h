...
<script>
  ...
  const PTZUpButton = document.getElementById('ptzup')
  const PTZDownButton = document.getElementById('ptzdown')
  const PTZLeftButton = document.getElementById('ptzleft')
  const PTZRightButton = document.getElementById('ptzright')
  const PTZCenterButton = document.getElementById('ptzcenter')
  const PTZPosX = document.getElementById('ptz_x')
  const PTZPosY = document.getElementById('ptz_y')
  const PTZStepSize = document.getElementById('servo_step')
  const PTZServo1Pin = document.getElementById('servo1_pin')
  const PTZServo2Pin = document.getElementById('servo2_pin')
  const hide = el => {
  ...
  const updateValue = (el, value, updateRemote) => {
    ...
    if (updateRemote && initialValue !== value) {
      ...
      if (el.id === "aec") {
        ...
        console.log('PTZ: up');
      } else if (el.id === "ptzdown") {
        console.log('PTZ: down');
      } else if (el.id === "ptzleft") {
        console.log('PTZ: left');
      } else if (el.id === "ptztight") {
        console.log('PTZ: right');
      } else if (el.id === "ptzcenter") {
        console.log('PTZ: center');
      } else if (el.id === "servo_step") {
        console.log('PTZ: servo_step: ' + value);
        PTZStepSize.value = value;
      } else if (el.id === "ptz_x") {
        PTZPosX.value = value;
        console.log('PTZ: ptz_x: ' + value);
      } else if (el.id === "ptz_y") {
        PTZPosY.value = value;
        console.log('PTZ: ptz_y: ' + value);
      } else if (el.id === "servo1_pin") {
        PTZServo1Pin.value = value;
        console.log('PTZ: servo1_pin: ' + value);
      } else if (el.id === "servo2_pin") {
        PTZServo2Pin.value = value;
        console.log('PTZ: servo2_pin: ' + value);
        ...
        console.log('min_frame_time.value ' + value);
  ...
  PTZUpButton.onclick = () => {
    if (parseInt(PTZPosY.value <180))
    PTZPosY.value = parseInt(PTZPosY.value) + parseInt(PTZStepSize.value);
    console.log("PTZPosY:", PTZPosY.value);
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/control?var=ptz&val=up", true);
    xhr.send();
  }
  PTZDownButton.onclick = () => {
    if (parseInt(PTZPosY.value >0))
    PTZPosY.value = parseInt(PTZPosY.value) - parseInt(PTZStepSize.value);
    console.log("PTZPosY:", PTZPosY.value);
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/control?var=ptz&val=down", true);
    xhr.send();
  }
  PTZLeftButton.onclick = () => {
    if (parseInt(PTZPosX.value <180))
    PTZPosX.value = parseInt(PTZPosX.value) + parseInt(PTZStepSize.value);
    console.log("PTZPosX:", PTZPosX.value);
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/control?var=ptz&val=left", true);
    xhr.send();
  }
  PTZRightButton.onclick = () => {
    if (parseInt(PTZPosX.value >0))
    PTZPosX.value = parseInt(PTZPosX.value) - parseInt(PTZStepSize.value);
    console.log("PTZPosX:", PTZPosX.value);
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/control?var=ptz&val=right", true);
    xhr.send();
  }
  PTZCenterButton.onclick = () => {
    PTZPosX.value = 90;
    PTZPosY.value = 90;
    console.log("PTZCenterButton");
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/control?var=ptz&val=center", true);
    xhr.send();
  }
  PTZPosX.onchange = () => {
    updateConfig(PTZPosX)
    console.log("PTZPosX:", PTZPosX.value);
  }
  PTZPosY.onchange = () => {
    updateConfig(PTZPosY)
    console.log("PTZPosY:", PTZPosY.value);
  }
  PTZStepSize.onchange = () => {
    updateConfig(PTZStepSize)
    console.log("PTZStepSize:", PTZStepSize.value);
  }
  PTZServo1Pin.onchange = () => {
    updateConfig(PTZServo1Pin)
    console.log("PTZServo1Pin:", PTZServo1Pin.value);
  }
  PTZServo2Pin.onchange = () => {
    updateConfig(PTZServo2Pin)
    console.log("PTZServo2Pin:", PTZServo2Pin.value);
  }
  swapButton.onclick = () => {
  ...
  rebootButton.onclick = () => {
    if (confirm("Reboot the Camera Module?")) {
      ...
      <style>@media(min-width:800px) and (orientation:landscape){#content{display:flex;flex-wrap:nowrap;align-items:stretch}}</style>
   </head>
   <body>
      <section class="main">
         <div id=logo>
         ...
         <div id=content>
            <div class=hidden id=sidebar>
               ...
               <nav id=menu>
                  ...
                  <div class=input-group id=set-xclk-group title="Camera Bus Clock Frequency&#013;Increasing this will raise the camera framerate and capture speed&#013;&#013;Raising too far will result in visual artifacts and/or incomplete frames&#013;This setting can vary a lot between boards, budget boards typically need lower values">
                     <label for=xclk>XCLK</label>
                     <div class=text>
                        <input id=xclk type=number min=2 max=32 step=1 class=default-action>
                  ...
                  <div  class="input-group,wrapper1" id=ptz-group>
                     <div id="controls">
                        <button id=ptzup class=default-action>Up</button>
                        <button id=ptzleft class=default-action>Left</button>
                        <button id=ptzcenter class=default-action>Center</button>
                        <button id=ptzright class=default-action>Right</button>
                        <button id=ptzdown class=default-action>Down</button>
                     </div>
                  </div>
                  <div class=input-group id=servo_step-group title="PTZ step angle">
                     <label for=servo_step>Servo step size</label>
                     <div class=text>
                        <input id=servo_step type=number min=1 max=30 step=1 class=default-action>
                     </div>
                  </div>
                  <div class=input-group id=ptz-pos-group title="PTZ absolute position X">
                     <label for=ptz-pos-group>PTZ absolute position</label>
                     <input id=ptz_x class=default-action>
                     <input id=ptz_y class=default-action>
                  </div>
                  <div class=input-group id=servo_pin-group title="PTZ servo GPIO pins ">
                     <label for=servo_pin>PTZ servo GPIO pins <span style="font-size:125%">⚠</span></label>
                     <input id=servo1_pin class=default-action>
                     <input id=servo2_pin class=default-action>
                  </div>
                  ...
                  <div class=input-group id=code_ver-group>
                     <label for=code_ver>
                     <a href=https://github.com/easytarget/esp32-cam-webserver title="ESP32 Cam Webserver on GitHub" target=_blank>Firmware</a></label>
            ...
            <figure>
               <div id=stream-container class="image-container hidden">
                  ...
                  <img id=stream src=data:, alt=stream>
               </div>
            </figure>
         </div>
      </section>
